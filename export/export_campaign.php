	...
	function start_process($dir_path,$dir_name){
		...
		foreach ($open_file as $value) {
			...
			if (!file_exists($value)) {
				...
				$file = $dir_path.$dir_name.'/export/ex_campaign.csv';
				//csv作成
				$fpcsv = fopen($file, 'w');
				$null_flg = 0;
				...
				if($null_flg === 0){/////////////////1行目nullならスルー　　0がOK状態
					...
					$temp = $dir_path.$dir_name.'/campaign.csv';
					$fp = fopen($temp, 'w');
					fwrite($fp, $data);
					fclose($fp);
					system('chmod 0777 '.$temp);

					output_log("".$value." : campaign.csvファイルの出力を完了\n");
					//exit(0);
				}
			}
		}
...
function readXlsx($readFile){
	...
	try{
		...
		$objPExcel = $objPExcel->setActiveSheetIndexByName("campaign");
