	...
	function start_process($dir_path,$dir_name){
		...
		foreach ($open_file as $value) {
			...
			if (!file_exists($value)) {
				...
				$file = $dir_path.$dir_name.'/export/ex_schedule.csv';
				//csv作成
				$fpcsv = fopen($file, 'w');
				$null_flg = 0;
				foreach($excel_arry as $key => $excel_data){
					...
					if($key === 7){
						$line = str_replace(PHP_EOL, '', $line);//置換
					}else{
				...
				if($null_flg === 0){/////////////////1行目nullならスルー　　0がOK状態
					...
					$temp = $dir_path.$dir_name.'/schedule.csv';
					$fp = fopen($temp, 'w');
					fwrite($fp, $data);
					fclose($fp);
					system('chmod 0777 '.$temp);

					output_log("".$value." : price.csvファイルの出力を完了\n");
					//exit(0);
				}
			}
		}
...
function readXlsx($readFile){
	...
	try{
		...
		$objPExcel = $objPExcel->setActiveSheetIndexByName("schedule");
